...
package net.floodlightcontroller.myspanningtree;
...
public class MySpanningTree
    ...
    private void writeFlowMod(IOFSwitch sw, OFFlowModCommand command, OFBufferId bufferId,
        ...
        fmb.setMatch(match);
        fmb.setCookie((U64.of(MySpanningTree.LEARNING_SWITCH_COOKIE)));
        fmb.setIdleTimeout(MySpanningTree.FLOWMOD_DEFAULT_IDLE_TIMEOUT);
        fmb.setHardTimeout(MySpanningTree.FLOWMOD_DEFAULT_HARD_TIMEOUT);
        fmb.setPriority(MySpanningTree.FLOWMOD_PRIORITY);
...
//		log.info("========================MAC ADDRESSES=======================");
//		log.info("SOURCE MAC: "+ sourceMac + "DEST MAC: "+ destMac);
        ...
        if (outPort == null) {
            ...
            if(inPort.getPortNumber()==1){
                for (Map.Entry<MacVlanPair, OFPort> entry: swMap.entrySet()) {
                    if(entry.getValue().getPortNumber()==2)
                        this.pushPacket(sw, m, pi, entry.getValue());
                }
            }else if(inPort.getPortNumber()==2){
                for (Map.Entry<MacVlanPair, OFPort> entry: swMap.entrySet()) {
                    if(entry.getValue().getPortNumber()==1)
                        this.pushPacket(sw, m, pi, entry.getValue());
                }
            }else {
                for (Map.Entry<MacVlanPair, OFPort> entry: swMap.entrySet()) {
...
//			for (Map.Entry<MacVlanPair, OFPort> entry: swMap.entrySet()) {
//				if (entry.getValue().equals(inPort)) {
//				}else{
//					log.info("+++++++++====================MAP VALUE==============+++++++++++");
//					log.info("inPort: " + inPort.getPortNumber() + "	outPort: " + entry.getValue().toString());
//					log.info("+++++++++===========================================+++++++++++");
//					this.pushPacket(sw, m, pi, entry.getValue());
//
//					this.writeFlowMod(sw, OFFlowModCommand.ADD, OFBufferId.NO_BUFFER, m, entry.getValue());
...
//								.setExact(MatchField.IN_PORT, entry.getValue());
...
//			this.writePacketOutForPacketIn(sw, pi, OFPort.FLOOD);
    ...
    private Command processFlowRemovedMessage(IOFSwitch sw, OFFlowRemoved flowRemovedMessage) {
        if (!flowRemovedMessage.getCookie().equals(U64.of(MySpanningTree.LEARNING_SWITCH_COOKIE))) {
            return Command.CONTINUE;
        }
        if (log.isTraceEnabled()) {
